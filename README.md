...
Concierge automatically creates an customisable “install” button that can be used by any Open Web App, using manifest file meta data to populate the component UI. Concierge will only display on browsers that support installable Open Web Apps, and only if the app has not yet been installed on a users device.
